...
export default function Hero({ gymName, phone, city }: HeroProps) {
  return (
    <div className="relative w-full min-h-[70vh] md:min-h-screen flex md:mt-15 py-10 items-center justify-center overflow-hidden px-6 bg-black/90 bg-cover bg-center bg-fixed">

      <div className="absolute inset-x-0 top-[28%] md:top-[10%] z-0 flex justify-center select-none pointer-events-none">
      ...
      <div className="absolute inset-x-0 bottom-0 h-[65%] z-15 pointer-events-none select-none mix-blend-screen opacity-70 group-hover:opacity-80 transition-opacity duration-300">
        <div className="absolute inset-0 md:bg-linear-to-t from-zinc-800 via-zinc-600/80 to-transparent blur-xl" />
        <div className="absolute inset-0 md:bg-linear-to-tr from-zinc-800/70 via-transparent to-transparent blur-2xl" />
      </div>
      <div className="absolute left-2 sm:left-6 md:left-8 bottom-3 sm:bottom-12 md:bottom-5 z-20 max-w-full sm:max-w-md transition-all duration-1000 pt-1 sm:pt-0 mt-0 sm:mt-2 opacity-100 translate-y-0 flex flex-col gap-2 justify-center px-3">
        <h1 className="text-white text-3xl sm:text-4xl md:text-6xl font-extrabold leading-[1.05]">
          PUSH YOUR <br className="hidden md:block"/>
          <span className="text-primary">LIMITS</span>
        </h1>
        <p className="text-gray-300 mt-1 text-lg sm:text-base md:text-base  sm:max-w-md leading-tight">
      ...
      <div className="absolute hidden md:block right-2 sm:right-6 md:right-16 bottom-4 sm:bottom-14 md:bottom-12 z-20 text-center sm:text-right transition-all duration-1000 mt-1 sm:mt-2 opacity-100 translate-y-0">
